...
└── src/
    ...
    ├── components/              # React 组件
    │   ├── index.ts             # 统一导出
    │   ├── AnalyticsProvider.tsx# 数据分析上下文（v2.0 新增）
    │   │
    ...
    │   │   ├── MobileAIFab.tsx         # AI 悬浮按钮（v1.9 新增）
    ...
    ├── hooks/                   # 自定义 Hooks
    │   ├── index.ts             # 统一导出
    │   ├── useAnalytics.ts      # 数据分析 Hook（v2.0 新增）
    ...
    │
    ├── lib/                     # 核心库
    │   ├── prisma.ts            # Prisma Client 实例（v2.0 新增）
...
| Hook | 说明 |
|------|------|
| `useAnalytics` | 数据分析 Hook（v2.0 新增）：自动追踪页面停留时长、心跳机制、sendBeacon 上报 |
...
### SQLite (Prisma ORM) - v2.0 新增

服务端数据存储，用于用户信息、认证和数据分析。
...
*文档版本：v2.4*  
*更新时间：2026-02-03*  
*更新内容：新增 v2.0 数据分析系统（Prisma/SQLite、analytics-service、useAnalytics Hook）*
