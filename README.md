...
- 开一个独立的「应用矩阵入口」，提供「小应用模式」承接高频上新。
- AI 对话作为统一调度入口：用户自然语言 -> 触发插件 -> 返回卡片和任务 -> 回写状态。
- 应用矩阵按学生需求分层，而不是按技术模块分层。

### 学生需求分层（决定优先级）

| 层级 | 目标 | 典型形态 | 价值判断 |
|------|------|----------|----------|
| **L0 必需层** | 不用就无法完成学习闭环 | 困惑补救（已实现）、复习计划（已实现）、行动清单回写（已实现） | 非用不可，用了离不开 |
| **L1 提效层** | 提升掌握速度与记忆稳定性 | 类 Duolingo 训练、类 Anki 间隔复习卡片 | 高频复用，稳定留存 |
| **L2 认知层** | 帮学生建立知识结构 | 思维导图、知识网络、错因聚类 | 提升迁移能力与理解深度 |
| **L3 传播层** | 提升分享意愿与自传播 | 学习战报、成就卡、同伴激励机制 | 社交货币属性，拉新增长 |
| **L4 实验层** | 低成本试错与创新 | 黑客松灵感应用、快速实验插件 | 快速验证，保留优胜者 |

说明：L0 的业务能力已具备，当前目标是迁移到插件统一协议，不重复造轮子。

### 入口与交互形态

1. 主入口：学生端新增「应用矩阵」按钮，进入小应用工作台。
2. 对话联动：AI 对话可直接调用应用插件，返回结构化卡片并可一键执行动作。
3. 统一协议：所有小应用统一走 `cards/tasks/trace` 协议，避免 UI 与业务逻辑耦合。

### 交互形态分层（GUI / Tool / Hybrid）

| 形态 | 适用场景 | 优势 | 风险 |
|------|----------|------|------|
| **GUI 应用** | 强视觉、强操作、强反馈的学习任务（如导图、卡片学习、训练面板） | 可见、可控、可沉浸 | 开发成本高，易过度设计 |
| **Tool 应用** | 后台执行、低交互、规则明确的能力（如检索、打分、规划、重排） | 上线快、复用强、可被 AI 组合调用 | 用户感知弱，价值不易被看见 |
| **Hybrid 应用** | 既要 AI 自动执行又要用户确认/微调（如困惑补救、复习计划、错题训练） | 效率与可控性平衡 | 边界设计不好会造成认知负担 |

默认策略：优先 `Tool-first` 打通能力，再补 `GUI-shell` 提升感知；核心闭环应用优先做 `Hybrid`。

### 交互形态决策四问

1. 这个任务是否需要学生做“可视化理解”？
2. 这个任务是否必须让学生“手动确认或编辑”？
3. 这个任务是否可以在后台自动执行且结果可验证？
4. 这个任务失败时，是否会直接影响学习闭环？

判定规则：
- Q1/Q2 高：偏 `GUI` 或 `Hybrid`
- Q3 高且 Q1/Q2 低：偏 `Tool`
- Q4 高：优先 `Hybrid`，保留用户确认

### 与 L0-L4 的映射建议

| 需求层 | 默认交互形态 | 说明 |
|------|---------------|------|
| **L0 必需层** | Hybrid | 既要自动产出，也要学生可确认、可回写 |
| **L1 提效层** | Hybrid / GUI | 训练与复习通常需要可视化进度与操作反馈 |
| **L2 认知层** | GUI | 导图/关系图天然依赖可视化 |
| **L3 传播层** | GUI + Tool | Tool 生成内容，GUI 负责展示与分享 |
| **L4 实验层** | Tool-first | 先低成本验证，再决定是否补 GUI |

### 迭代节奏（先稳后快）

1. 先接通 L0 必需层（困惑补救 + 复习计划），替换 Action 数据源，不改主 UI 结构。
2. 再上线 L1 提效层（卡片学习/间隔复习）验证留存。
3. 然后补 L2 认知层（导图/关系图）验证理解迁移效果。
4. 最后用 L3/L4 持续做增长与实验。

### 当前落地状态（2026-02-13）

- 已在复习模式接入应用矩阵入口（桌面 + 移动端）。
- 已落地首个闭环插件：知识卡片工作台（`cards/tasks/trace` 协议）。
- 已实现“应用内回放”与主时间轴统一 seek 行为（同一时间值归一规则）。
- 已完成工作区状态持久化抽离，支持切页/刷新后恢复复习上下文。
- 已补最小 Playwright 回归集，形成“改动后可自动兜底”的测试护栏。
- 当前保持“对话主交互 + 应用矩阵执行增强层”并存，不替代原有对话流。
