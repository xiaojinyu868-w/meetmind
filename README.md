...
│  │  │  • Zustand (全局状态)                       │                 │   │
│  │  │  • localStorage (时间轴、锚点)              │                 │   │
│  │  │  • IndexedDB / Dexie (音频、大数据)         │                 │   │
│  │  └──────────────────────┬──────────────────────┘                 │   │
│  └─────────────────────────┼───────────────────────────────────────┘   │
│                            │                                           │
│  ┌─────────────────────────┼───────────────────────────────────────┐   │
│  │                    服务端 (server.js + Next.js API)              │   │
│  │                         │                                        │   │
│  │  ┌─────────────┐  ┌─────────────┐  ┌──────────────┐              │   │
│  │  │ WebSocket   │  │ /api/chat   │  │ /api/tutor   │              │   │
│  │  │ ASR 代理    │  │ /api/transcribe│ /api/upload  │              │   │
│  │  └──────┬──────┘  └──────┬──────┘  └──────┬───────┘              │   │
│  └─────────┼────────────────┼────────────────┼──────────────────────┘   │
│            │                │                │                          │
│  ┌─────────┴────────────────┴────────────────┴──────────────────────┐   │
│  │                    阿里云百炼 (DashScope)                         │   │
│  │  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐   │   │
│  │  │ 实时 ASR         │  │ 批处理 ASR       │  │ LLM (通义千问)   │   │   │
│  │  │ qwen3-asr-flash-│  │ qwen3-asr-flash │  │ qwen3-max       │   │   │
│  │  │ realtime        │  │                 │  │ qwen-plus       │   │   │
│  │  └─────────────────┘  └─────────────────┘  └─────────────────┘   │   │
│  └──────────────────────────────────────────────────────────────────┘   │
│                                                                         │
...
├── docker-compose.yml              # Open Notebook 服务配置
...
└── src/
    ...
    │   ├── layout.tsx              # 根布局
    │   ├── page.tsx                # 学生端首页
    │   ├── parent/page.tsx         # 家长端页面
    │   ├── teacher/page.tsx        # 教师端页面
    │   └── api/                    # API 路由
    │       ├── asr-config/         # ASR 配置
    │       ├── chat/               # 通用对话
    │       ├── transcribe/         # 语音转录
    ...
    │
    ├── components/                 # React 组件
    │   ├── Recorder.tsx            # 🎙️ 录音组件（核心）
    │   ├── AITutor.tsx             # 🤖 AI 家教对话
    │   ├── AIChat.tsx              # 💬 自由对话
    │   ├── TimelineView.tsx        # 📊 课堂时间轴
    │   ├── WaveformPlayer.tsx      # 🎵 波形播放器
    │   ├── ActionList.tsx          # ✅ 行动清单
    │   ├── ConfusionHeatmap.tsx    # 🔥 困惑热区
    │   └── ...                     # 其他组件
    │
    ├── hooks/                      # 自定义 Hooks
    │   └── useAudioSessions.ts     # 音频会话管理
    │
    ├── lib/                        # 核心库
    │   ├── config.ts               # 应用配置
    │   ├── db.ts                   # IndexedDB 操作
    │   ├── services/               # 业务服务层
    │   │   ├── anchor-service.ts       # 困惑点标注
    │   │   ├── dashscope-asr-service.ts # 实时 ASR
    │   │   ├── llm-service.ts          # 多模型 LLM
    │   │   ├── memory-service.ts       # 课堂时间轴
    │   │   ├── parent-service.ts       # 家长端服务
    │   │   ├── tutor-service.ts        # AI 家教
    │   │   └── ...                     # 其他服务
    │   └── longcut/                # LongCut 核心算法
    │       ├── quote-matcher.ts        # 引用匹配
    │       ├── timestamp-utils.ts      # 时间戳工具
    │       └── transcript-sentence-merger.ts # 句子合并
    │
    ├── types/                      # TypeScript 类型
    │   ├── index.ts                # 核心类型定义
    │   └── dify.ts                 # Dify 相关类型
...
| `npm run dev:next` | 仅启动 Next.js（不含 WebSocket） |
| `npm run build` | 生产构建 |
| `npm run start` | 生产运行 |
