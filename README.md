...
>
> MVP 2.8.1 - 把课堂"变成可回放、可定位、可追溯的时间轴记忆"
...
#### v2.8.1 - 移动端模型列表可用性优化（2026-02-18）

| 功能 | 描述 |
|------|------|
| **小屏弹层稳定展示** | 模型列表在小屏下改为底部弹层展示，避免右侧弹出被裁切或显示不全 |
| **视口约束优化** | 弹层宽高按移动端视口约束，保障 360~430px 宽度下完整可读 |
| **模型项排版重排** | 标题与描述间距、行高、截断策略优化，降低拥挤感，提升可扫描性 |
| **本次范围控制** | 本版本仅保留移动端模型列表优化，其它实验性优化不纳入交付 |
