...
| `runtime.ts` | `runDraw(chunks, options)`：`cleanScript`（剥 ```js 围栏 / `<script>`）→ 注入 API 执行 → render；`console.log` 收进 `log` 返回 |
...
- 脚本超时（死循环）由 `draw-runtime-client.ts` 2s 终止并重建 Worker；错误文本经 `onIssue` 记入会话，学生下次开口时以「板上情况」附给老师（只进模型上下文，不进课堂记录），老师自己改。
