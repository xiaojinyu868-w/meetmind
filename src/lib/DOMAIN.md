...
| `learning/` | 2 | 学习记忆的纯模型，客户端与服务端共用：`mastery-trail-model.ts`（检验结果 → 掌握状态：还没稳 / 刚记住 / 已经稳了）、`moment-title.ts`（给课堂里的一个时刻起名：学生备注 → 脉络要点 → 老师那一刻原话首句；复习页困惑点列表 / 问同学书桌 / 同桌开场 chip 同一套命名） |
