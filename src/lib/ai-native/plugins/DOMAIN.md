...
| `board-blocks.ts` | 结构化板书块动作类型（shape / table / line / code，teach 新引擎全量词表）+ `codeTextToLines` helper；从 board-script.ts 拆出守 300 行类型文件限制，BoardAction 联合仍在 board-script.ts 聚合并再导出。块不占 wN 编号（countPageWrites 只数 write），elementId 为引擎语义 id（spotlight / laser / wb_edit_code 的定位键，服务端 ensureElementId 保证存在），与 write 一样受 flattenPage 的 clear 截断 |
