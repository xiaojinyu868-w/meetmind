...
export const COPY = {
  ...
  wordExplainer: {
    explain: '解释一下',
    dialogLabel: (selection: string): string => `解释「${selection}」`,
    initialPrompt: (selection: string): string => `请解释「${selection}」`,
    stop: '停止生成',
    close: '关闭',
    analyzing: '正在分析…',
    requestFailed: '这次没有接上，内容还在。',
    attachmentPrompt: '请结合我附上的材料解释这个选词。',
    dropFiles: '松开以添加图片',
    removeFile: '移除',
    parsing: '解析中…',
    upload: '上传图片或文件',
    uploadHint: '上传图片 / 也可拖入或粘贴',
    followUp: '继续追问…',
    send: '发送',
  },
  ...
  globalAsk: {
    ...
    contextTasksOverviewTitle: '学习任务',
    contextTasksOverviewEmpty: '还没有需要接着推进的学习任务。',
    ...
    taskTitle: '学习任务',
    taskCount: (count: number): string => `${count} 条`,
    taskStatusActive: '正在进行',
    ...
    taskComplete: '这条学完了',
    taskReopen: '重新打开',
    taskUpdated: (date: string): string => date ? `更新于 ${date}` : '最近更新',
    taskEvidence: (lessons: number, activities: number): string => {
      const parts = [
        lessons > 0 ? `${lessons} 节课堂` : '',
        activities > 0 ? `${activities} 次练习` : '',
      ].filter(Boolean);
      return parts.join(' · ');
    },
    taskEvidenceAction: '查看这条线的学习现场',
    taskEvidenceCollapse: '收起学习现场',
    taskEvidenceHeading: '这条线由这些现场推进',
    taskEvidenceUnavailable: '现场记录还没回到这台设备，但任务进展已经保留。',
    taskEmpty: '开始一次“陪我学会”后，MeetMind 会把进展和下一步留在这里。',
    threadTitle: '上次还在继续',
    threadResume: '接着学',
  ...
  conversationHistory: {
    loadListFailed: '加载对话列表失败',
    createFailed: '创建对话失败',
    missing: '对话不存在',
    loadFailed: '加载对话失败',
    noSelection: '没有选中的对话',
    addMessageFailed: '添加消息失败',
    deleteFailed: '删除对话失败',
    searchFailed: '搜索对话失败',
    initializeFailed: '初始化对话失败',
    notInitialized: '对话未初始化',
    tutorTitle: (date: string): string => `困惑点辅导 ${date}`,
    chatTitle: (date: string): string => `AI 对话 ${date}`,
  },

  tutor: {
    emptyAfterName: '在这里。',
    reviewStarters: ['先讲清这节课的主线', '从我标记的地方开始'] as const,
    historyLoading: '正在接回上一轮对话…',
    historyRestored: '已接回上一轮对话',
    historyNew: '新对话',
    historyViewing: (title: string): string => `正在查看：${title}`,
  },
  ...
  collection: {
    ...
    sourceImport: {
      videoReadyEmpty: '视频已导入，但转写为空，请更换视频或重试。',
      transcribingLater: '转写稍后完成',
      recognizing: '正在识别…',
      seeingClearly: '正在看清楚…',
      reading: '正在阅读…',
      recognizingText: '正在识别文字…',
      readingLongContent: '内容比较多，还在读…',
      understandingLongContent: '内容比较长，还在努力…',
      almostDone: '快好了，再等等…',
      audioOnly: '这次只接收音频文件',
      supportOnly: '这次只接收资料文件',
      unsupportedFile: (label: string, name: string): string => `${label} 暂时还不能自动接入：${name}`,
      transcriptionFailed: (message: string): string => `转写未完成：${message}`,
      videoLink: '视频链接',
      articleLink: '图文链接',
      providerVideoLink: (provider: string): string => `${provider} 链接`,
      providerArticleLink: (provider: string): string => `${provider} 文章`,
      genericVideoProvider: '网页视频',
      fetchingVideo: '正在获取视频信息…',
      downloadingAudio: '正在下载音频…',
      understandingContent: '正在听懂内容…',
      linkParseFailed: '链接解析失败',
      videoContentEmpty: '导入成功但没听到内容，稍后再试试',
      importFailed: '导入失败',
      importFallback: '这条链接先收下了，稍后再试试',
      extractingArticle: '正在提取文章内容…',
      articleExtractFailed: '文章提取失败',
      articleContentEmpty: '文章内容为空，稍后再试试',
      videoErrors: {
  ...
  lesson: {
    ...
    actionUpcoming: '准备上课',
    digest: {
      processingTitle: '正在整理',
      processingBody: '同桌正在把这节课整理成笔记…',
      failed: '笔记暂时没有生成',
      failedBody: '原声和转录还在，等一会儿可以再整理一次。',
      retry: '再整理一次',
      viewTranscript: '查看转录原文',
      original: '查看原文',
      hideOriginal: '收起原文',
      supplementalTitle: '课后补充',
      supplementalBody: '课后收下的资料，没有对应到课堂具体时间。',
    },
  },
  ...
  companion: {
    ...
    errorRetry: '重新连接',
    conversation: {
      inlineAppNotEnough: '课堂内容还不够，再听一会儿再试。',
      inlineAppNotSuitable: '这段内容不适合做成这个学习方式，可以换一种。',
      inlineAppGenerateFailed: '这次没做好，稍后可以再试。',
      flashcardAgain: (front: string, back: string): string => `这张没记住没关系——\n\n**${front}**\n\n${back}`,
      explainDifferently: '换个讲法',
      explainDifferentlyPrompt: (front: string): string => `上面这张闪卡（${front}）我还是记不住，能换个角度讲讲吗？`,
      flashcardsReviewed: (count: number): string => `${count} 张过完了。明天再来一遍，记得更稳。`,
      lessonContextNotEnough: '等你录完一节课，我们再好好聊——现在我对你这节课还没听够。',
      noLessonContext: '我还没听过你的课，不知道你想聊哪方面。先录一节，我就有话说了。',
      answerFallback: '嗯……我对这节课还没理解到能接这个问题的程度。再给我点时间，或者换个具体点的问法？',
      networkDefault: '我这边网络好像不太好',
      networkEmpty: '网络不太好',
      networkUnavailable: '网络好像不通',
      answerFailed: (reason: string): string => `我这边没接上（${reason}），待会儿再问我一次？`,
    },
    composer: {
      attachmentOnlyPrompt: '请结合我附上的材料，帮我讲清楚。',
      largePasteAttached: '内容比较长，已经作为附件放进来了',
      largePasteHint: '同学会读完整段后再回答',
      dropFiles: '松开以添加图片或文件',
      removeFile: '移除附件',
      parsing: '正在读取…',
      retryUpload: '再试一次',
      offlinePlaceholder: '网络断开了，内容会留在这里',
      busyPlaceholder: '可以先写下一句…',
      upload: '上传图片或文件',
      uploadHint: '上传图片或文件，也可以拖入或粘贴',
      stop: '停止回答',
      stopShort: '停',
      send: '发送',
    },
  ...
  recording: {
    sourceTitle: (time: string): string => `录音 ${time}`,
    transcriptionPending: '转写稍后回来',
    noTranscribableAudio: '录音结束时没有拿到可转写音频',
  ...
  reviewAnchor: {
    ...
    momentLabel: '课中记下',
    confusionLabel: '困惑点',
    saved: (timestamp: string): string => `已记下 ${timestamp}`,
    savedHint: '下课后可以从时间轴直接回到这一段',
    duplicate: '这一段已经记过了',
    unavailable: '这节课还没准备好，稍后再记一次',
    saveFailed: '这次没记下来，再试一次',
    selectTitle: '选择一个课堂标记',
    selectHint: '点击时间轴或波形上的标记查看附近原文',
    resolved: '已解决',
    unresolved: '待解决',
    locate: '定位',
    playContext: '播放片段',
    resolve: '标记已解决',
    contextTitle: '该时刻前后的课堂内容',
    contextWindow: (seconds: number): string => `前后 ${seconds}s`,
    noTranscript: '暂无该时间段的转录内容',
    notePlaceholder: '补一句你当时为什么记下这里…',
    confusionNotePlaceholder: '记录你对这个困惑点的理解或疑问…',
    saveNote: '保存笔记',
    cancel: '取消',
    addNote: '添加笔记',
    note: '备注',
    nextConfusion: '跳转下一个困惑点',
    close: '关闭',
  },
  ...
  apps: {
    ...
    matrix: {
      ...
      demoRecommendationReason: '这段听力里有可独立回忆的表达和词义，先用闪卡练一遍。',
  ...
  feedback: {
    ...
    typeLabel: '反馈类型',
    types: {
      bug: { label: '问题反馈', description: '报告功能异常或错误' },
      feature: { label: '功能建议', description: '提出功能或体验建议' },
      question: { label: '使用咨询', description: '咨询产品使用方法' },
      other: { label: '其他', description: '告诉我们其他想法' },
    },
    titleLabel: '标题',
    titlePlaceholder: '请简要描述问题或建议',
    detailLabel: '详细描述',
    detailPlaceholder: '请描述你遇到的情况、操作步骤，以及你原本期待看到的结果。',
    contactLabel: '联系方式',
    contactPlaceholder: '邮箱或手机号，方便我们与你联系',
    optional: '（可选）',
    requiredError: '请填写标题和详细描述',
    success: '反馈已收到',
    successHint: '我们会尽快查看并处理。',
    submitFailed: '这次没提交成功，请稍后再试',
    networkError: '网络没有接上，请稍后再试',
    submit: '提交反馈',
    submitting: '正在提交',
    otherContact: '其他联系方式',
    email: '邮箱：originedu@meetmind.online',
    replyTime: '我们通常会在 1-3 个工作日内回复',
  },

  allNotes: {
    footerHint: '每一条笔记，都在让下一次复习更轻松。',
  },
