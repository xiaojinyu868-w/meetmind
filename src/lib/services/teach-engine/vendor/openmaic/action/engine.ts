...
// [ADAPT vs upstream] 上游 import '@/lib/api/stage-api'（UI 持久化层）；
// 主路实现为 runtime/stage-store.ts（每线程内存 stage store，构造注入 recorder）。
import type { StageStore } from '../../../runtime/stage-store';
import { createStageAPI } from '../../../runtime/stage-store';
// [ADAPT vs upstream] 上游 import '@/lib/store/{canvas,whiteboard-history,media-generation}'
// 模块级 zustand 单例（一页一课堂）；主路多线程并发会串板，改为构造注入
// （见 runtime/board-stores.ts 与本类 constructor 的 stores 参数）。
import type { BoardStores, MediaTask } from '../../../runtime/board-stores';
import { createBoardStores } from '../../../runtime/board-stores';
// [ADAPT vs upstream] 上游 '@/lib/utils/audio-player' → runtime/audio-pacer.ts。
import type { AudioPlayer } from '../../../runtime/audio-pacer';
// [ADAPT vs upstream] 上游 '@/lib/types/action' 本就是 @openmaic/dsl 的重导出
// （+ LegacySpeechAction 兼容型）；直接指向 dsl，LegacySpeechAction 就地定义。
import type { SpeechAction as DslSpeechAction } from '../dsl';
/** Compatibility shape for server classroom payloads created before audioUrl was removed. */
type LegacySpeechAction = DslSpeechAction & { audioUrl?: string };
import type {
  Action,
  SpotlightAction,
  LaserAction,
...
import type { CodeLine, PPTVideoElement } from '../dsl';
// [ADAPT vs upstream] 上游 '@/lib/media/media-task-resolution'（slide 视频媒体绑定
// 解析）；主路 v1 无 slide/视频场景，内联 undefined stub。
interface VideoMediaTaskResolution<T> {
  task: T | undefined;
  [key: string]: unknown;
}
function resolveVideoMediaForElement(
  _tasks: Readonly<Record<string, MediaTask>>,
  _element: unknown,
  _stageId?: string,
): VideoMediaTaskResolution<MediaTask> | undefined {
  return undefined;
}
// [ADAPT vs upstream] 上游 '@/lib/choreography' 即纯转发。
import {
...
// [upstream 保留] '@/lib/logger' 恰好命中主仓库真实 logger（src/lib/logger.ts）。
...
export class ActionEngine {
  ...
  // [ADAPT vs upstream] 板上/特效/媒体 store 从模块级单例改为构造注入（多线程隔离）。
  private stores: BoardStores;
  ...
  constructor(
    ...
    this.stores = stores ?? createBoardStores();
  }
  ...
  clearEffects(): void {
    ...
    this.stores.canvas.getState().clearAllEffects();
  }

  resetPlaybackVisualState(): void {
    this.clearEffects();
    this.stores.canvas.getState().pauseVideo();
    this.stores.canvas.getState().setWhiteboardOpen(false);
    this.stores.canvas.getState().setWhiteboardClearing(false);
  ...
  private executeSpotlight(action: SpotlightAction): void {
    this.stores.canvas.getState().setSpotlight(action.elementId, {
  ...
  private executeLaser(action: LaserAction): void {
    this.stores.canvas.getState().setLaser(action.elementId, {
  ...
  private async executePlayVideo(
    ...
    const resolveBinding = () =>
      resolveActionVideoMedia(
        this.stageStore,
        this.stores.mediaGeneration.getState().tasks,
        action.elementId,
      );
    ...
    if (binding) {
      ...
      if (task && task.status !== 'done') {
        ...
        await new Promise<void>((resolve) => {
          ...
          unsubscribe = this.stores.mediaGeneration.subscribe((state) => {
    ...
    if (options.signal?.aborted) return;
    this.stores.canvas.getState().playVideo(action.elementId);
    ...
    return new Promise<void>((resolve) => {
      ...
      const abortPlayback = () => {
        if (this.stores.canvas.getState().playingVideoElementId === action.elementId) {
          this.stores.canvas.getState().pauseVideo();
        }
        finish();
      };
      ...
      unsubscribe = this.stores.canvas.subscribe((state) => {
  ...
  private async ensureWhiteboardOpen(options: ActionExecutionOptions = {}): Promise<void> {
    if (!this.stores.canvas.getState().whiteboardOpen) {
  ...
  private async executeWbOpen(options: ActionExecutionOptions = {}): Promise<void> {
    ...
    this.stores.canvas.getState().setWhiteboardOpen(true);
  ...
  private async executeWbClear(options: ActionExecutionOptions = {}): Promise<void> {
    ...
    this.stores.history.getState().pushSnapshot(wb.data.elements!);

    // Trigger cascade exit animation
    this.stores.canvas.getState().setWhiteboardClearing(true);
