...
export const wechatAuthService = {
   ...
   */
  getAuthUrl(redirectUri?: string, scope: WechatOauthScope = 'snsapi_base', explicitState?: string): string {
    cleanExpiredStates();

    const state = explicitState || generateState();
  ...
  async login(code: string, state: string): Promise<AuthResponse> {
    ...
    return wechatIdentityService.login({
      openId,
  ...
  async bindToUser(userId: string, code: string, state: string): Promise<{ success: boolean; error?: string }> {
    ...
    return wechatIdentityService.bind({
      userId,
      openId,
      accessToken: tokenResponse.access_token,
      refreshToken: tokenResponse.refresh_token,
