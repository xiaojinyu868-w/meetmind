...
| `learner-context-service.ts` | ~170 | 「这个学习者」读槽（renewal plan §6 读侧）：`resolveLearnerContext`（远端 `CONTEXT_SYSTEM_URL` → 本机切片 → 空；登录用户才问远端，1.5 s 超时静默回落）、`parseLearnerContext`（zod，只认当前 v）、`formatLearnerContextForPrompt`（切片 → ≤600 字事实段落：还没稳 → 刚记住 → 已经稳 → 困惑 → 最近 → 在学）。本机供给方在 `components/learner-context-local.ts`；类型在 `types/learner-context.ts` |
