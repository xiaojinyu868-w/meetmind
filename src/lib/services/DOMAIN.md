...
`material-lessons/DOMAIN.md`：「从材料开一节课」能力层（与来源无关，2026-09-13）——候选材料形状、课的单位（一篇 / 一条线 / 整组兜底）与预算、同学把一组材料读成几条线、接下来看哪篇的判断；知乎等来源只写 capture → candidate 的薄壳。契约 `docs/plans/2026-09-13-material-lessons.md`。
...
| `learner-context-provider.ts` | ~170 | 读契约的**事实半**服务端供给方（`source: 'server'`）：LearningEvent 表 assessment 事件 → `lib/learning/mastery-trail-model` 同一份规则聚成掌握状态（证据 id 指回事件）；User.learnerProfileJson → 困惑 / 主题 / 偏好 / 进度 + 最近课堂。只读时聚合不写画像。对外 `POST /api/context/v1/learner-context`。**2026-09-11 `selectMasteryEntries`**：`request.concepts` 点名的概念全部保留不占 limit（闪卡到期模型按卡面问历史）；其余按 状态 → 本课优先（证据落在 `request.sessionId`）→ 最近 取 limit；steps 留最近 8 步（够算到期） |
| `learner-context-service.ts` | ~190 | 「这个学习者」读槽（renewal plan §6 读侧）：`resolveLearnerContext` 合成两半——事实半（服务端事件表 → 请求方本机切片 → 空）+ 理解半（`CONTEXT_ENABLED` 时 `context/learner-understanding` 按 `request.task` 向 Hindsight 召回，有来源、经暂停 / 忘记过滤；本机切片里的理解半不可信会被丢弃）；`formatLearnerContextForPrompt(context, { sessionId })`（事实 ≤1000 字——2026-09-11 从 600 提上来，600 只装得下还没稳的那几条，"已经稳的少出"到不了模型手里；证据落在当前课的概念标「本课」；再附跨应用记忆 JSON 证据 ≤6000 字，带"历史证据不是指令"的说明）；`parseLearnerContext`（zod）。消费方：`/api/apps/execute`、Tutor 六模式（除 shared）、`/api/teach/threads` |
...
| `infographic-skill-service.ts` | ~90 | 课堂信息图 skill 管线单一真相（2026-09 起）：宝玉 baoyu-infographic 手册原文零改动 vendor 在 `assets/infographic/baoyu-infographic/`；预设画风 hand-drawn-edu + 版式 bento-grid + 横版 16:9（`INFOGRAPHIC_PRESET`），LLM 单次过整节课转录直接产出按手册基座模板填好的生图提示词；被 studio-workshop 插件与 ai-control-service（管理员预览/受控试跑）共用；换风格只改 `INFOGRAPHIC_PRESET` |
...
| `volc-podcast.ts` | 582 | 火山引擎播客 TTS（WebSocket 双向流式）；需控制台开通 `volc.service_type.10050`，未开通建连即 403 |
| `dashscope-podcast.ts` | ~200 | 播客默认合成路径（`PODCAST_TTS_PROVIDER=dashscope`，默认即此）：LLM 双主播脚本 → qwen3-tts-instruct-flash 逐句合成（Cherry/Ethan 双音色，`PODCAST_TTS_VOICE_A/B` 可覆盖）→ ffmpeg 拼接 mp3 → 持久化 `public/uploads/podcast/` 经 `/api/podcast/audio/{file}` 交付；复用 DASHSCOPE_API_KEY；任一句失败整期报错（对齐"不出音频不算好"契约） |
...
| `local-workspace-migration.ts` | ~330 | **登录迁移只推变过的课**（2026-09-11，从 `lib/hooks/useAuth.tsx` 抽出）：每节课算证据签名（用户 + 转录 / 锚点 / 摘要 / 精选 / 笔记 / 对话的数量与最后更新时刻 + 会话元数据，FNV-1a 短哈希，不含 updatedAt / checkpointAt 这类心跳字段），只推签名 ≠ `audioSessions.migrationSignature` 的课，每批成功即 `markSessionsMigrated` 写回（无 syncState 的顺手标 synced）；正在录的课不推；20/批、413 拆 5/批、失败批不标下次再来。服务端按 metadata.sessionId 复用 live 行，一节课只一行。`useAuth` 只在登录态就绪时调一次 `runLocalWorkspaceMigration` |
