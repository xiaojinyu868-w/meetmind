...
| `learner-context-provider.ts` | ~130 | 读契约的服务端供给方（`source: 'server'`，外部 context 系统合并前的参考实现 / 合并后的对照物）：LearningEvent 表 assessment 事件 → `lib/learning/mastery-trail-model` 同一份规则聚成掌握状态（证据 id 指回事件）；User.learnerProfileJson → 困惑 / 主题 / 偏好 / 进度 + 最近课堂。只读时聚合不写画像。对外 `POST /api/context/v1/learner-context` |
| `learner-context-service.ts` | ~170 | 「这个学习者」读槽（renewal plan §6 读侧）：`resolveLearnerContext`（远端 `CONTEXT_SYSTEM_URL` → 服务端事件表 `learner-context-provider` → 本机切片 → 空；登录用户才问前两者，1.5 s 超时静默回落）、`parseLearnerContext`（zod，只认当前 v）、`formatLearnerContextForPrompt`（切片 → ≤600 字事实段落：还没稳 → 刚记住 → 已经稳 → 困惑 → 最近 → 在学）。本机供给方在 `components/learner-context-local.ts`；类型在 `types/learner-context.ts` |
