...
| `zhihu-lesson-service.ts` | 开一节 live 课。**课的单位**（`ZhihuLessonMode`，09-13 起）：`single` 一篇讲透（全文 ≤12000 字直接给，再长 `excerptFor` 按结构取到 8000 并附 `outlineOf` 目录）/ `theme` 一条线 2–4 篇（每篇 ≤4000，`selectNamedMaterials` 保持点名顺序）/ `favlist` 整夹（旧：`selectLessonMaterials` 按赞同 ≤8 篇 1800/1200/700）→ 只给进包的几篇 `materializeCaptures` → `buildMaterialPack`（`mode` / `pickReason` / `ownerUserId` / `priorLessons`）→ `createThread(engine='live')` + `writeLiveMaterials`；`listLessonsForUser`（含 `sourceIds` / `mode`，按篇记课）。`excerptOf` 按结构取：开头 + 各小节首段 + 结尾，预算有余轮着补后续段 | Firecrawl credit 只花在进材料包的几篇上；single 的课题 = 文章标题，其余默认收藏夹名；priorLessons single 按同一篇、其余按同一收藏夹；本目录不 import teach-live 内部，只认识它的 `LiveMaterialPack` 通用形状 |
| `zhihu-theme-service.ts` | 同学读一个收藏夹：`themesForFavlist` 按"放在一节课里讲得通"分成几条线 + misc + 最值得先讲的 `start`；一次 tutorQuick 调用只看标题 / 类型 / 作者 / 摘要；`buildThemePrompt` 用编号代替 id 防幻觉、`normalizeThemes` 对回真实 id（一篇只属一条线、未知编号丢、没归线的进 misc、start 必须可讲）；按条目集合 `themesHash` 落盘缓存 | 坏 JSON / 超时退回"一条线 = 全部可讲"+ 赞同最高的当起点，页面照样能用；≤1 篇可讲不叫模型；不抓正文 |
