...
| `live-materials.ts` | 学生自带材料开课（2026-09-12，知乎线首用）：`LiveMaterialPack` 落盘 `data/teach-materials/<threadId>.json`（`TeachConfig.materialsDir`；文件即事实，不改 schema），`ensureSession` 建会话时 `readLiveMaterialsBlock` 拼进 system prompt 的「材料」段（`buildTeachLivePrompt` 第三参数；没有材料一字不加）。段里写清讲法：从材料讲不另讲一套、分歧点名对比、口播说「材料 1」板书写 [A1]、只有摘要的只当线索、课题从共同主题起；`priorLessons`（同一组材料之前上过的课，来源方填）非空时多一句「已经上过 N 节：《…》，别从头讲同一段」。谁来挑材料 / 抽正文 / 查之前的课是来源方的事（`services/material-lessons` 能力层 + `services/zhihu/zhihu-lesson-service.ts` 适配）；`mode`（single / theme / favlist）决定材料块措辞（一篇讲透 vs 一条线连起来）、`pickReason` 让老师开口先说为什么先讲它 |
| `lesson-record.ts` | **课的物化**（2026-09-13）：事件日志（每行带 ts）→ `LessonRecord`——老师 / 学生两个说话人的转录段（时间轴按语速 220ms/字铺，不用生成速度；首条「开始上课」不算学生说话；口播里「材料 1 / 材料一」→ `sourceRef` A1，单篇课默认 A1）、scene → 页 + 板书统计、轮次与是否讲完、材料（coverage full-text / outline / summary、点到没点到、≤3500 字节选 `capExcerpt`）、`priorLessons` 一行、回舞台地址（按 pack.source 决定哪张舞台）、`digest` 与 `lessonRecordDigestLine`。路由 `GET /api/teach/threads/[id]/record`（需登录；材料包有 ownerUserId 的只本人可读）。客户端存课见 `lib/db/lesson-records.ts` + `hooks/useLessonRecordSync`；复习页消费见 `hooks/useLessonRecord` + `components/review/LessonMaterialsCard` + `lib/review/lesson-materials-context` | 事件日志是唯一事实源，每次现算不落快照；record 只含这节课自己的 + 材料包 + 之前课一行，收藏夹里别的不进（`docs/plans/2026-09-13-material-lessons.md` §4） |
...
| `__tests__/lesson-record.test.ts` | 事件日志 → record：说话人 / 时间轴 / 页 / 轮次 / 材料引用与 coverage / 目录 / digest；无材料包与被打断的情形 |
