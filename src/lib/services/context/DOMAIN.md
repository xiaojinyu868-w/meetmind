...
| `education-adapter.ts` | 旧学习事件到通用原始观察的适配；角色、来源与不确定性保留在内容中。三种载荷：对话（conversation.turn）、活动（learning.activity，可带完整 observation）、应用矩阵的结构化检验结果（practice.assessment：概念 × 结果 + 判分依据 `learner_self_report` / `application_answer_match`，2026-09-09）。开关启用后与旧 `LearningEvent` 表**双写**（learning-observation-service），不再二选一 |
| `learner-understanding.ts` | 「这个学习者」读槽的理解半（2026-09-09）：owner principal → 最近一条 active 个人经历作 afterEventId → `prepareContext` → `LearnerUnderstanding`（预算内 JSON 证据 + 来源句柄 + degraded）；任何失败返回 null。主链路：`learner-context-service.resolveLearnerContext` → 应用矩阵 / Tutor 六模式（除 shared）/ teach 开课 |
| `tutor-adapter.ts` | M1 形态的 global Tutor 追加入口，现委托 `learner-understanding`（同一 helper）；tutor 路由已改走 LearnerContext 理解半（六模式除 shared，含 global），本文件保留供直接调用方与「最新经历优先」回归测试 |
