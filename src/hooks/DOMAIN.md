...
| `useWorkspaceContextLoader.ts` | ~230 | 工作区上下文加载 + 同步（API 加载 + captures 合并 + captureDrivenPulse 自动过期），从 page.tsx 提取（Phase 5）。**2026-09-10 起会刷新**：首次加载后页面回前台 / 窗口聚焦 / 恢复联网（≥20s 一次）与每 60s 定时各拉一次 `/api/workspace/current`，同一条合并 + 回填链路（按 id / sourceKey 去重，回填不覆盖本机编辑）——此前只在 (userId, wechatToken) 变化时拉一次，另一台设备结束的课不刷新页面永远看不到 |
...
| `useClassroomLessons.ts` | ~330 | 课堂列表数据适配（audioSessions + transcripts + highlightTopics + workspaceEchoes/Captures + preferences + sourceItems → Lesson[] + markReviewed），响应式；把当前 Recorder 正在录的 sessionId 传给 lessonAdapter（只有它是活动条，其余 `status='recording'` 的行显示「还没结束」）；挂载时**不再**把 recording 态一刀改成 completed（改由 useUnfinishedRecordings 承接）；登录后挂载 / 恢复联网 / 回前台时跑 `retryPendingRecordingUploads` + `syncPendingRecordings` |
...
| `useFlashcardsReview.ts` | ~150 | **闪卡跨会话间隔复习的读侧编排（2026-09-11）**：一叠卡（正面）→ `lib/learning/spaced-review-model.planReview` 的复习计划 + 进入态那一句。历史两半：本机 `device-outcomes`（挂载时读，`OUTCOMES_UPDATED_EVENT` 一响就重读——刚打完分的一轮立刻进计划）；登录用户再 POST `/api/context/v1/learner-context` 点名这叠卡的正面（`concepts`）拿跨设备 steps，同一张卡服务端为准、本机只补服务端最后一步 60s 之后的（`mergeReviewHistory`）；30s 进程内缓存。消费方：FlashcardsWindow（顺序在开始翻牌前冻结）、WorkshopYellowPage 闪卡路径卡副题 |
