...
export function MiniPlayer({
  ...
  return (
    ...
    >
      ...
      {/* 当前时间 */}
      <span className="flex-shrink-0 text-xs font-medium text-[var(--dedao-text)] tabular-nums w-10">
      ...
      >
        ...
        <div className="absolute inset-x-0 h-1 bg-[#E0D6CC] rounded-full overflow-hidden">
          {/* 已播放进度 */}
          <div
            className="h-full bg-[var(--dedao-gold)] rounded-full transition-all duration-100"
          ...
          return (
            <button
              ...
              className={cn(
                ...
                marker.resolved 
                  ? 'bg-[var(--dedao-resolved)]' 
                  : 'bg-[var(--dedao-confusion)]'
              )}
              style={{ left: `${position}%` }}
              title={marker.resolved ? '已解决' : '待解决'}
            />
          );
        })}
