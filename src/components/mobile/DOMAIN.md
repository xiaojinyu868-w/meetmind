...
- `MobileAppRunner.tsx` 是移动端应用执行器：六类应用共用 catalog 与 AppRenderSurface；移动端矩阵不再维护缺少播客/信息图的四项手写目录。矩阵首张推荐卡保持横向完整说明，中间应用采用双列紧凑卡；当推荐卡造成奇数尾项时，最后一张自动收成横卡，禁止留出像“少加载一个应用”的空格。推荐只改变排序与强调，不隐藏可选项。
