...
| `MobileAppShell.tsx` | **移动端统一页面壳**：接入真实数据，渲染各 screen。首页顶部可挂「有一节课没结束」恢复条（`unfinishedLessons` 等 props 由 page.tsx 的 useUnfinishedRecordings 传入；「继续录」先等 `onResumeUnfinished` 收好前半段，再走壳自己的 onStartRecording 进录课页）。「最近收下」为空时（2026-09-09）与桌面收集空态同一套语言但更轻：一句标题 + 副题 + 能力 pill（课件 / 链接 / 想法 / 语音 / 微信），不重复上方行动卡里的 Octo 与录课。零内容首屏直接展示“录课 / 放资料 / 问 MeetMind”三条路径；开始录课必须等待 Recorder 返回成功后才 push 录课页，避免权限失败时出现假的 `00:00` 或旧课转录。内置 90 秒试听用真实音频时钟驱动转录与课堂脉络渐进出现，英文试听默认 EN→中，结束后进入同一套课后笔记和应用门禁。录课中已经形成的课堂脉络在课后应用矩阵直接显示为已生成成果，打开后保留完整推进并可回跳原话；其余七类应用（含精讲页 explainer）统一走 `CatalogAppScreen → MobileAppRunner`，共享标题、数据源和失败恢复体验。真实短材料继续由 readiness 控制可生成范围。是否为零内容同时读取收集、笔记总结、记忆、近期活动和活跃学习线程；录课页复用 `useClassroomFlow + ClassroomFlowCanvas`，默认展示课堂脉络并可切回原话；AI 对话通过 render slot 接收 page.tsx 传入的 SafeAITutor。首页把活跃学习线和“最近收下”的原件明确分层；无昵称时只显示中性个人图标，不伪造用户身份。管理员开启管理视图时只在头像上显示微小状态点，开关仍收在设置页，不新增悬浮工具栏压住手机内容。 |
...
| `MobileLessonPath.tsx` | 手机端课后学习页（2026-09-08，课后学习页 v2 的手机形态）：此前是 8 个应用平铺 + 一枚「推荐」角标；现在是「这节课（标题 + 时长 / 标记 / 难点）→ 先做这一件（带学生能核对的理由，页面唯一饱和主按钮）→ 学习路径（检验 → 记住 → 讲出来 → 带走，每步写结果 "8 张记住 6" 或 还没开始 / 做好了；2026-09-10 删掉标题旁的路径说明句）→ 还可以这样学（安静列表）→ 走完了（wrap）」。与桌面读同一份判断（`lesson-path-model`）和会话层结果（`review-session-outcomes`）；「已做好」按 `readCachedAppResult` 本机缓存判定；锚点直接读 capture editor store（与 segments 同源，不按 sessionId 过滤——示例课锚点带 DEMO_SESSION_ID 而会话键是 guest-demo）。`AppsScreen` 只管导航与头部 |
...
- 移动首页第一屏只允许一个主叙事：学习控制台。采集方式是入口层，AI 产物是能力层，今日发现和最近内容是上下文层；不要再让拍照、速记、情报卡和最近列表同时争夺主视觉。控制台使用明亮的 `pine-fog + card` 分层，深色只用于文字，主动作使用 `pine`；不回到大面积纯黑面板（v6 时代实测：像工具台，不像"发给同学"）。顶栏不重复日期等低价值小字；主叙事下也不再加一段解释，动作本身必须让用途可理解。
