...
| `GuestSyncHint.tsx` | ~35 | 访客本机有录好的课时的一句安静入口（2026-09-11）：「登录后，这节课会跟着你到任何设备 · 登录」→ `/login?next=/app`，与 UnfinishedLessonBar 同一套语言（白底 hairline、左侧细柱用 ink/20——既不是发生中也不是等决定）；显隐由 `hooks/useGuestSyncHint` 决定；桌面挂在 ClassroomView 的 noticeSlot（恢复条下方）、移动端 MobileAppShell 首页恢复条下方 |
...
| `ClassroomRecordingView.tsx` | ~640 | 转录卡头部（2026-09-08）一行状态：LIVE 徽标 + 正在听这一句 / 已记 N 句 / 等老师开口——此前两行（"课堂文字"+状态）在窄栏里标题截成"课堂…"、LIVE 逐字换行；示例播放键图标化，只在需要手势出声时露文字；收尾卡文案走 COPY.recording.afterClass（不再对用户说"应用矩阵"）；录课中视图（宽桌面左侧实时文字 + 中间课堂脉络；移动端和中等宽度桌面在“脉络 / 原话”之间切换，避免三栏硬挤；含翻译与试听课音频控制）。试听课默认 EN→中，音频结束后只引导点击“结束这节课”，由上层切到课后复习页 / 应用矩阵；转录卡头部有呼吸球仪式（listening 时存在、试听完成即消散）和返回课程列表的退出入口（试听课先暂停音频，真实录音由活动条承接可回来），句首时间戳统一 `.cite-ts`；DemoAfterClassPanel 为 v7 收尾卡（纸感 + pine CTA + Octo happy + 一次柔光扫过）。**2026-09-11**：头部一行状态读 `capture-editor-store.liveAsrLink`，实时字幕链路 reconnecting / offline 时改说「实时字幕暂时断开，录音仍在继续」/「实时字幕已停下…」（`data-testid=live-captions-notice`），恢复后回到正在听 / 已记 N 句 |
...
## 设计判断（每条来自真实用户观察；想推翻，拿同一场景下的新证据来）

- **95% 克制 + 少数仪式时刻情绪化**（`docs/PRODUCT_TASTE.md` 原则层；具体 token / 投影档位看 `docs/DESIGN_SYSTEM.md` 当前版）
...
- **听准优先于分人**：首页与录课中不让用户选择 ASR 供应商或“单人 / 多人”模式；实时字幕固定走高准确率 Qwen，realtime 单遍即定稿（2026-08）。说话人分离不再自动跑；手动「重新精转」时至少两位稳定发言证据成立才显示匿名“发言者 A / B”，不自动猜老师或真实姓名。
