...
## 刻意不做的事（这一态的产品边界，括号里是理由）

- ✗ 不写 IndexedDB / conversationService（目标共建是一次性的对话，不持久化历史；长期结果走学习记忆管线）
- ✗ 不接 SharedAgent（这是个人态，个人上下文默认私有）
- ✗ 不做 viral 机制（"分享你的目标"会把私密的学习意图变成社交表演）
- ✗ 不替用户做决定 / 不催促 / 不弹任务清单（教练式对话的本质——它只在答案会改变学习路径时才问）
