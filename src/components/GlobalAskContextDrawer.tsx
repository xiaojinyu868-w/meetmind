  ...
  return (
    <div className="absolute inset-0 z-30 flex justify-end bg-ink/10" role="presentation" onMouseDown={onClose}>
      <aside
        ...
        aria-label={GLOBAL_ASK_COPY.contextRailTitle}
        onMouseDown={(event) => event.stopPropagation()}
      >
        <header className="flex items-start justify-between gap-4 border-b border-divider px-5 py-5 sm:px-6">
          <div>
            <p className="font-mono text-[9.5px] font-semibold uppercase tracking-[0.12em] text-pine">{GLOBAL_ASK_COPY.contextRailEyebrow}</p>
            <h2 className="mt-1.5 text-[17px] font-semibold text-ink">{GLOBAL_ASK_COPY.contextRailTitle}</h2>
            <p className="mt-1 max-w-xs text-[11.5px] leading-5 text-ink-muted">{GLOBAL_ASK_COPY.contextRailBody}</p>
          </div>
          <button type="button" onClick={onClose} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-ink-muted hover:bg-paper-warm hover:text-ink" aria-label={GLOBAL_ASK_COPY.close}>
            <X size={15} />
          </button>
        </header>

        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-2 sm:px-6">
          {!hasContext ? <p className="py-8 text-[13px] leading-6 text-ink-secondary">{GLOBAL_ASK_COPY.contextEmpty}</p> : null}

          {currentCount > 0 ? (
            <section className="border-b border-divider py-5">
              <div className="flex items-center gap-2 text-[12px] font-semibold text-ink"><BookOpen size={14} className="text-pine" />{GLOBAL_ASK_COPY.contextDrawerCurrent}</div>
              {sources.length > 0 ? (
                <div className="mt-2">
                  {sources.map((source) => (
                ...
                <p className="mt-2 text-[11.5px] leading-5 text-ink-muted">{GLOBAL_ASK_COPY.contextCurrentCount(currentCount)}</p>
              )}
            </section>
          ) : null}
          ...
          {recentCount > 0 ? (
            <section className="flex items-start gap-3 border-b border-divider py-5">
              ...
              <div><p className="text-[12px] font-semibold text-ink">{GLOBAL_ASK_COPY.contextDrawerRecent}</p><p className="mt-1 text-[11.5px] leading-5 text-ink-muted">{GLOBAL_ASK_COPY.contextRecentCount(recentCount)}，{GLOBAL_ASK_COPY.contextDrawerRecentBody}</p></div>
            </section>
          ) : null}
          ...
          {memoryCount > 0 ? (
            <section className="flex items-start gap-3 border-b border-divider py-5">
              ...
              <div><p className="text-[12px] font-semibold text-ink">{GLOBAL_ASK_COPY.contextDrawerMemory}</p><p className="mt-1 text-[11.5px] leading-5 text-ink-muted">{GLOBAL_ASK_COPY.contextMemoryCount(memoryCount)}，{GLOBAL_ASK_COPY.contextDrawerMemoryBody}</p></div>
            </section>
          ) : null}

          {activeThread?.status === 'active' ? (
            <section className="py-5">
              <p className="font-mono text-[9px] uppercase tracking-[0.1em] text-pine">{GLOBAL_ASK_COPY.contextActiveThread}</p>
        ...
        <div className="border-t border-divider p-4 sm:px-6">
          <button type="button" onClick={onOpenMemory} className="flex w-full items-center justify-between rounded-[14px] bg-ink px-4 py-3 text-[12px] font-medium text-white transition hover:bg-pine">
            <span className="inline-flex items-center gap-2"><History size={13} />{GLOBAL_ASK_COPY.memoryAction}</span>
