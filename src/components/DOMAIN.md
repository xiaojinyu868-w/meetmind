...
| `GlobalAskDesk.tsx` / `global-ask-desk.ts` | ~95 / ~180 | 问同学空态的「书桌」（2026-09-08）：composer 上方几行可点的实物——正在读（当前课堂点名 / 打开的材料，超出折 +N）、你标的（锚点按"没跟上优先、时间顺序"，取消的不上桌）、还没稳（掌握轨迹里非 stable 的概念，还没稳先于刚记住）、最近（仅无当前课堂时）、记得（长期理解里困惑先于主题；偏好 / 长处不上桌）。点一件 → 一句指向具体位置的问题落进输入框（"00:30 那里我没跟上，从这讲一下"）。全空显示空桌面 + 访客试听入口（`resetDemoEntryConsumed` 后跳 `/app?guest=1&entry=demo`；登录用户无此入口，文案不提）。桌上有实物时收起泛化建议卡（深度模式句式不同，保留）。纯函数只陈述学生留下的事实，不推断；测试 `global-ask-desk.test.ts` |
