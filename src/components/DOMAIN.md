...
| `MasteryTrailSection.tsx` / `mastery-trail.ts` | ~70 / ~125 | 「我的上下文」总览的掌握轨迹（2026-09-08）：读本机会话层检验结果（`review-session-outcomes` 的 `mm-review-outcomes:*`），按概念聚合成事实序列（测验 ✕ → 闪卡 ✓），状态只有三个词（还没稳 / 刚记住 / 已经稳了；只对过一次不敢说稳），还没稳排最前，无记录整块不渲染。`conceptLabel` 把整句问题收成术语（引号内优先，否则截断）供桌面等窄处使用。数据边界：现在读本机，外部 context 系统合并后同一形状由它供给 |
