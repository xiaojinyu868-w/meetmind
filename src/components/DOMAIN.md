...
| `MasteryTrailSection.tsx` / `mastery-trail.ts` | ~70 / ~25 | 「我的上下文」总览的掌握轨迹（2026-09-08）：读本机会话层检验结果（`review-session-outcomes` 的 `mm-review-outcomes:*`），按概念聚合成事实序列（测验 ✕ → 闪卡 ✓），状态只有三个词（还没稳 / 刚记住 / 已经稳了；只对过一次不敢说稳），还没稳排最前，无记录整块不渲染。概念展示名用 `lib/utils/concept-label.ts` 的 `conceptLabel`（引号内术语优先，否则截断；书桌、轨迹、课后路径的推荐理由三处同一规则，mastery-trail 只 re-export）。纯模型在 `lib/learning/mastery-trail-model.ts`（客户端与服务端共用同一份规则；服务端 `learner-context-provider` 用它聚 LearningEvent 表）；本组件只读本机会话层结果，登录用户换设备由服务端切片供给。数据走 `hooks/useMasteryTrail`（本机 + 登录用户的服务端切片合并，换设备也看到同一个自己）；`mastery-trail.ts` 只剩 re-export，模型在 `lib/learning/mastery-trail-model.ts` |
