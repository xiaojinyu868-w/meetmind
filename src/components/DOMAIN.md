...
| `MasteryTrailSection.tsx` / `mastery-trail.ts` | ~70 / ~125 | 「我的上下文」总览的掌握轨迹（2026-09-08）：读本机会话层检验结果（`review-session-outcomes` 的 `mm-review-outcomes:*`），按概念聚合成事实序列（测验 ✕ → 闪卡 ✓），状态只有三个词（还没稳 / 刚记住 / 已经稳了；只对过一次不敢说稳），还没稳排最前，无记录整块不渲染。概念展示名用 `lib/utils/concept-label.ts` 的 `conceptLabel`（引号内术语优先，否则截断；书桌、轨迹、课后路径的推荐理由三处同一规则，mastery-trail 只 re-export）。数据边界：现在读本机，外部 context 系统合并后同一形状由它供给 |
