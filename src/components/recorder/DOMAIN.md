...
| 文件 | 职责 |
|------|------|
| `recorder-types.ts` | 录音器配置/状态/props 类型（含 process.env 默认值、`RecorderAudioSource` 重导出）；`headless` 只挂载录音引擎，不输出隐藏的可聚焦 UI；`recorderRef` 供 `dynamic()` 外壳用普通 prop 传递 imperative handle |
