...
- `WorkshopYellowPage.tsx` + CSS module 是应用矩阵首屏基线：用户选择“带走重点 / 检验理解 / 记住核心”等学习动作；六项稳定能力始终可见，每卡说明适用场景、时间投入和产物。中间窄工作区使用约 80–96px 高的能力缩略卡，主操作保持紧凑，避免六张大卡形成工具黄页；手机端再展开为 44px 可点击操作。`/api/apps/readiness` 的推荐只改变首屏优先级，不裁剪完整能力：材料充足时六项都可生成；材料有限时只启用有证据支撑的低风险应用，其余卡片保留但暂不可用；材料过短、非学习内容或转录不可靠时六项仍作为能力预览存在，但全部禁止生成，并清理该 session 已有的派生缓存（不碰原录音），避免旧幻觉在内容变化后复现。判断必须同时带真实原文、材料标题与来源类型，避免把听力练习等对话型学习材料误判为闲聊。分享只在完成至少一个可分享且当前内容允许的产物后出现。首次生成留在矩阵后台完成；视觉默认 `canvas/card/ink/divider`，不使用渐变和装饰性阴影。
