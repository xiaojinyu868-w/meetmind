...
- 2026-09-10 交互打磨：卡片与按钮 `:active` scale（80–150ms，reduced-motion 关掉）；做完一轮回到路径时结果行按内容 key 重挂载 + `mm-rise-in` 260ms 浮出、「先做这一件」按应用 key 重新进场、「走完了」卡进场；应用完成态里的 `NextStepCard` 220ms 进场、按钮 ≥40px。
- 课后学习页「开始」= 应用立刻出现（2026-09-08，`WorkshopYellowPage.startAndOpen`）：按下即 `onOpenAppWindow`，窗口先显示 AppWindowPlaceholder 的"同学在听"生成态，结果落缓存后经同页事件刷新；自动打开过的运行做好后不再 toast。信息图 / 播客仍是做好即弹预览。此前是后台生成 → 卡片"正在做…" → toast「做好了 · 打开」，页面唯一主动作以一条 toast 收尾。
- `useAppExecution` 按 session + app 缓存成品；普通重做失败时可保留旧成品，但服务端返回 `CONTENT_NOT_READY` 代表当前范围本身不成立，必须同时清掉旧结果与缓存，禁止出现“没做好”却仍可分享旧成品的矛盾。`GENERATION_FAILED`（2026-09-09：模型两次都没做出可用成品，插件不再用模板题 / 模板卡 / 抽样片段凑数）进失败态，`AppRenderSurface` 在分发前统一渲染 `AppWindowPlaceholder status=error` + 「再试一次」，不写任何记忆事件状态。
