...
export default function LearnerOnboarding({ onComplete, onSkip }: LearnerOnboardingProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(247,247,245,0.85)', backdropFilter: 'blur(8px)' }}>
      <div className="w-full max-w-md mx-4 rounded-2xl p-8" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E9E9E7' }}>

        {/* 进度指示 */}
        <div className="flex items-center justify-center gap-2 mb-6">
          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: '#232322' }} />
          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: step === 2 ? '#232322' : '#E9E9E7' }} />
        </div>

        {/* Step 1: 选身份 */}
        {step === 1 && (
          <>
            <h2 className="text-xl font-semibold text-center mb-1" style={{ color: '#232322' }}>
              你现在是
            </h2>
            <p className="text-sm text-center mb-6" style={{ color: '#787774' }}>
              帮助 MeetMind 更懂你的学习
            </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              {STAGES.map((s) => (
                <button
                  ...
                  className="flex flex-col items-center gap-1.5 p-4 rounded-xl transition-colors hover:bg-gray-50"
                  style={{ border: '1px solid #E9E9E7' }}
                >
                  <span className="text-2xl">{s.icon}</span>
                  <span className="text-sm font-medium" style={{ color: '#232322' }}>{s.label}</span>
                  <span className="text-xs" style={{ color: '#A3A39E' }}>{s.desc}</span>
                </button>
              ))}
            </div>
        ...
        {step === 2 && stage && (
          <>
            ...
            <h2 className="text-xl font-semibold mb-1" style={{ color: '#232322' }}>
              {STAGES.find(s => s.key === stage)?.icon} {STAGES.find(s => s.key === stage)?.label}
            </h2>
            <p className="text-sm mb-5" style={{ color: '#787774' }}>
              填完就好，10 秒的事
            </p>
            ...
            <div className="space-y-4">
              {stage === 'k12' && (
                <div>
                  <label className="block text-sm font-medium mb-1.5" style={{ color: '#232322' }}>年级</label>
                  <div className="flex flex-wrap gap-2">
                    {K12_GRADES.map(g => (
                      <button
                        ...
                        style={{
                          backgroundColor: gradeLevel === g ? '#232322' : '#F7F7F5',
                          color: gradeLevel === g ? '#FFFFFF' : '#232322',
                          border: `1px solid ${gradeLevel === g ? '#232322' : '#E9E9E7'}`,
                        }}
                      >
                        {g}
                      </button>
              ...
              {stage === 'university' && (
                <>
                  <div>
                    <label className="block text-sm font-medium mb-1.5" style={{ color: '#232322' }}>专业</label>
                    <input
                      type="text"
                      value={major}
                      ...
                      style={{ backgroundColor: '#F7F7F5', border: '1px solid #E9E9E7', color: '#232322' }}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1.5" style={{ color: '#232322' }}>年级</label>
                    <div className="flex flex-wrap gap-2">
                      {UNIVERSITY_YEARS.map(y => (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg text-sm transition-colors"
                          style={{
                            backgroundColor: year === y ? '#232322' : '#F7F7F5',
                            color: year === y ? '#FFFFFF' : '#232322',
                            border: `1px solid ${year === y ? '#232322' : '#E9E9E7'}`,
                          }}
                        >
                          {y}
                        </button>
              ...
              {stage === 'graduate' && (
                <div>
                  <label className="block text-sm font-medium mb-1.5" style={{ color: '#232322' }}>研究方向</label>
                  <input
                    type="text"
                    value={field}
              ...
              {stage === 'working' && (
                <>
                  <div>
                    <label className="block text-sm font-medium mb-1.5" style={{ color: '#232322' }}>所在行业</label>
                    <input
                      type="text"
                      value={industry}
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1.5" style={{ color: '#232322' }}>学习目标</label>
                    <input
                      type="text"
                      value={learningGoal}
              ...
              {/* 所有阶段共享——学习是多线程的 */}
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#787774' }}>
                  还在学什么？
                  <span className="font-normal text-xs ml-1" style={{ color: '#A3A39E' }}>选填</span>
                </label>
                <input
                  type="text"
            ...
            <button
              ...
              style={{
                backgroundColor: canSubmit ? '#232322' : '#E9E9E7',
                color: '#FFFFFF',
              }}
            >
              {saving ? '保存中...' : '开始使用'}
