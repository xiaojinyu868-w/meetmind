 ...
 * 不传 concepts 但传了 threadId：从这节课复习页的交卷记录（LearningEvent assessment，sessionId = teach:<threadId>）里推没稳的概念；
 * 一条交卷记录都没有 → 返回 { groups: [], noAssessment: true }，页面提示先做一套题。
 *
...
import { weakConceptsForSession } from '@/lib/services/zhihu/weak-concepts-from-assessments';
...
export async function POST(request: NextRequest) {
  ...
  let concepts = Array.isArray(body?.concepts) ? body.concepts.filter((c): c is string => typeof c === 'string' && c.trim().length >= 2).slice(0, 5) : [];
  const threadId = typeof body?.threadId === 'string' && /^[a-zA-Z0-9_-]+$/.test(body.threadId) ? body.threadId : null;
  if (!concepts.length && threadId) {
    concepts = await weakConceptsForSession(auth.userId, `teach:${threadId}`).catch(() => []);
    if (!concepts.length) return NextResponse.json({ success: true, groups: [], exhausted: false, noAssessment: true, concepts: [] });
  }
  if (!concepts.length) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, groups, exhausted, noAssessment: false, concepts });
  } catch (error) {
