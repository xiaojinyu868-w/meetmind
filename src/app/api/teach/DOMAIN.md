...
| `/api/teach/threads/[id]/draw-fix` | POST | **仅 live**：`<draw>` 脚本自愈 `{chunks, index, error, segmentId?}` → `{script, verified}`（模型只修出错的那一段，服务端复跑验证；每线程 40 次）。验证通过且带 `segmentId` 时以 `draw-fix` 事件 `{segmentId, script, error}` 落线程事件日志（不广播、不进课堂记录 / 模型历史；回看与恢复时替换原段，否则重放的是跑不通的原脚本） |
