# api/teach/ —— AI 家教「上课」线（codex 底座 × teach-engine 双线并存）

> 交互式教学会话：浏览器 ←SSE→ 本目录路由 → 按线程引擎归属分发到两套编排：
...
> 引擎归属在创建线程时按 `TEACH_ENGINE` 快照进 `TeachThread.engine`
> （codex | engine）；**engine=null 的迁移前旧线程一律按 codex 分发**。
> 路由是薄壳：分发只改 threads/messages/interrupt 三个写路由，stream /
> events / internal/* 零改动（事件契约两侧完全一致）。
...
{type:'image-ready',id,url}      插图回填完成（**仅 codex 线程产出**：engine v1 词表无 image 动作；url 为 /uploads/teach/ 本地静态路径）
...
| `/api/teach/threads` | POST | 新建课程 `{topic}`（≤100字，先只支持文本课题）→ `{thread}`；按 `TEACH_ENGINE` 快照 `engine` 字段，preflight 按目标引擎分发 |
...
- `TeachThread`（prisma）：id/title/topic/model/codexThreadId/engine/status/createdAt/updatedAt。
  codexThreadId 首个 turn 拉起后回填（仅 codex 线程），进程回收/重启后 thread/resume 续讲。
  engine 是创建时按 `TEACH_ENGINE` 快照的引擎归属；null = 迁移前旧线程，一律走 codex。
