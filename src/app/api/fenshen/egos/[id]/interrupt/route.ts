 ...
 *
 * body: {text?}。收到即 turn/interrupt；附带 text 时，等 interrupted 落地后
 * 同线程以学生消息续讲（上下文保留）。事件（interrupted → 新 turn 的
 * text-delta…）同样经 GET .../stream 流出。
 */
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    await interruptFenshenChat(params.id, text);
