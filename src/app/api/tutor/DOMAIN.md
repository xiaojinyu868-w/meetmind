...
| `intent/route.ts` | `POST /api/tutor/intent`：将原始问题与学习上下文整理为 `LearningIntentPlan`；可返回 1-3 个动态选择题，前端回传 `answers[{questionId, question, optionIds, optionLabels}]` 后得到最终计划；高置信且无问题的计划由前端直接执行，只有真实歧义或较低置信度才停下来确认；不持久化、不替用户确认 |
