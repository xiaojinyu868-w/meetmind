...
const HELP_SECTIONS: HelpSection[] = [
    ...
    items: [
      {
        ...
        answer: 'MeetMind 会帮你把课堂、资料和问题先收好，再整理成复习内容。你像发微信一样把学习现场发进来，它会慢慢变成笔记总结、复习材料和学习同桌。\n\n当前聚焦课堂场景：录一节课 → MeetMind 帮你听懂这节课 → 生成一张适合分享到班级群的课堂笔记图。'
      },
      {
        question: 'MeetMind 和录音转写工具有什么区别？',
        answer: '录音转写只是第一步。MeetMind 不只给你一堆文字，而是：\n\n1. 把课堂原件先收好\n2. 整理重点、时间线和困惑点\n3. 生成笔记总结、随堂检验和学习应用\n\n简单说，转写工具给你文字，MeetMind 给你一个「听过你课的学习同桌」。'
      },
      {
        question: '我的数据安全吗？',
  ...
  },
  {
    ...
    items: [
      ...
      {
        question: '如何通过微信收集？',
        answer: 'MeetMind 支持微信服务号收集：\n\n1. 关注 MeetMind 微信服务号\n2. 首次使用需要绑定你的 MeetMind 账号\n3. 绑定后，把文字、语音、图片、链接直接发给服务号\n4. 内容会自动进入你的收集流\n\n你随手发，它在后面帮你整理。'
      },
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      {
        question: '学习同桌和普通聊天有什么不同？',
        answer: '学习同桌不是泛泛回答问题，而是围绕你的真实课堂继续讲：\n\n- 它听过这节课，会按课堂内容回答\n- 它记得你的困惑，会围绕不理解的地方解释\n- 它顺着你的材料，不会脱离上下文\n\n不是从题库开始，而是从你这节课的真实内容开始。'
      },
      {
        question: '如何使用学习同桌？',
        answer: '有几种方式打开学习同桌：\n\n1. 课堂页右侧面板：录课时随时可以问\n2. 复习页中：就正在复习的这节课提问\n3. 收集页中：选中材料后继续提问\n\n你可以自由输入问题，也可以使用快捷按钮：先讲核心 / 换成例子 / 拆成步骤 / 提炼要点。'
      },
      {
        question: '什么是课堂同桌？',
        answer: '课堂同桌是课堂页右侧常驻的学习面板。它不是空白聊天框等你发话，而是「进来就看到它已经做了点什么」：\n\n- 课前：提醒你这节课的相关信息\n- 录课中：进入「正在听课」模式，偶尔给出一个预感\n- 课后：轻声提示这节课有什么值得注意的\n\n它的消息是「放下」的，不是「弹出」的。'
      },
    ]
  },
  {
    ...
    items: [
      {
        question: '什么是笔记总结？',
        answer: '笔记总结是 MeetMind 从你的学习内容中整理出的知识反馈。它不是长报告，也不是待办清单。\n\n它的特点：\n- 安静：不通知、不弹窗、不催促\n- 小：一次只讲一个发现\n- 有根：每句话都能指回真实原件\n- 不急：理解成熟后自然出现\n\n它会在收集流中自然出现，不需要你额外操作。'
      },
      {
        question: '课堂笔记图是什么？如何分享？',
        answer: '课堂笔记图是一张可以分享的图片卡：\n\n- 包含课程名、日期、核心概念、老师金句、一句话总结\n- 点击笔记卡片上的「分享」按钮即可生成\n- 长按图片可以保存到手机\n- 适合分享到班级群，让同学也看到这节课的核心。'
      },
    ]
  },
  {
    ...
    items: [
      {
        question: '有哪些学习应用？',
        answer: '应用矩阵围绕六种学习动作展开：\n\n- 带走重点：把定义、公式和易错点压成一页速查表\n- 检验理解：完成可作答测验，提交后查看反馈与课堂证据\n- 记住核心：用闪卡主动回忆，再按掌握情况继续练\n- 看清结构：把主干、分支和关系整理成可交互导图\n- 换种方式再听：把课堂重讲成双人对话播客\n- 做成一张图：把最值得带走的内容变成可分享的信息图\n\n进入课后复习页的应用矩阵，选择此刻最需要的一种即可。'
      },
      {
        question: '如何使用学习应用？',
        answer: '1. 进入一节课的复习视图\n2. 中间是「接下来怎么学」：先看同桌建议的“先做这一件”和它的理由\n3. 学习路径按检验 → 记住 → 讲出来 → 带走排好，点“开始”，同桌会在后台做好\n4. 做好后点“打开”练习、阅读或回看证据；每一步的结果会记在路径上，下一步会带上它\n5. 需要时可以再做一版，原课堂内容不会被改动。'
      },
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      {
        question: '桌面端是什么？如何安装？',
        answer: '桌面端是一个常驻电脑的小应用：屏幕角落住着桌宠 Octo，主窗口里跑着完整的 MeetMind。它多做三件网页版做不到的事：直接收电脑播放的声音（录网课不用带机器人入会）、口袋——任何应用里选中一段按一下就收进来（带格式、带来源）、桌宠随时互动。\n\n安装：\n1. 在官网首页下载对应版本（macOS 分 Apple 芯片 / Intel，Windows 需 10 及以上 64 位）\n2. 安装包暂未签名：macOS 首次请「右键 → 打开」，Windows 在 SmartScreen 提示中选「更多信息 → 仍要运行」\n3. 打开后在主窗口里登录一次，和网页版是同一个账号\n\n主窗口关掉不会退出，MeetMind 会常驻后台；真正退出请用托盘的 Octo 图标菜单。'
      },
      {
        question: '桌宠 Octo 有哪些玩法？',
        answer: 'Octo 住在屏幕角落，没人理它时会呼吸、眨眼，深夜会犯困睡着；眼睛会跟着你的光标转。\n\n- 单击：打开口袋——今天收进来的东西一条流，记一句、问同学、截一块屏\n- 双击：开始或停止「旁听」，把电脑正在播放的课录下来\n- 右键：最小菜单（打开主窗口、退出等）\n- 拖动：给它换个位置，松手会有果冻一样的回弹\n- 把文字、网址、网页图片或图片文件拖到它身上：直接收进口袋\n\n连续戳它，它会从开心、惊讶到假装生气。'
      },
      {
        question: '怎么用桌面端录网课？',
        answer: '两种方式：\n\n1. 双击桌宠开始「旁听」，再双击结束——最快的方式，不用打开任何窗口\n2. 在主窗口的课堂页点「开始一节课」，声音来源选「电脑声音」或「两路都录」\n\n桌面端收电脑声音是系统级授权，不会像浏览器那样每次弹窗让你勾选。旁听时把鼠标悬停在桌宠身上，可以看到已经录了多久。\n\n录完的课和网页版完全打通：课后复习、笔记、应用矩阵都在主窗口里用。'
      },
      {
        question: '口袋怎么用？（选中、按一下，就收进来）',
        answer: '在任何应用里——ChatGPT 网页、PDF、Word、微信——选中一段，按 Ctrl/⌘ + Shift + M：\n\n- 选中了文字：带格式收下（加粗、列表、代码块、公式都保留），并记下它从哪个应用、哪个网页来\n- 没选中、但剪贴板里有一张图（比如刚用截图工具截的）：收图\n- 什么都没有：进入框选，拖一块屏幕收下；Esc 取消，双击整屏\n- 正在录课时按：当前画面作为关键帧挂到这节课的时间轴上\n\n收下的瞬间光标旁会出一张小回执，点「撤销」就收回；桌宠会做一个吞食的小动作。\n\nmacOS 第一次收文字会请求「辅助功能」权限（模拟一次复制需要）；不给也行，先自己复制再按热键同样能收。\n\n网络不好时先存在本地，下次启动自动补传，不会丢。收进来的东西在 Ctrl/⌘ + Shift + K 的口袋窗里按来源成组，同学读过之后会出现在今日情报和问同学里。'
      },
      {
        question: '有哪些全局热键？',
        answer: '在任何应用里都能用：\n\n- Cmd/Ctrl + Shift + M：收下面前的东西——选中的文字 / 剪贴板里的图 / 框选一块屏（录课中则把当前画面挂到课堂时间轴）\n- Cmd/Ctrl + Shift + K：唤起 / 收起口袋窗（失焦会自动收起）\n\n被别的应用占用时会自动退到 Cmd/Ctrl + Alt + M / K，托盘菜单里显示的是当前真正生效的热键；想改成别的，托盘「打开设置文件」里改 hotkeyCapture / hotkeyPocket 后重启即可。'
      },
      {
        question: '桌面端会自动更新吗？',
        answer: '会。每次启动后桌面端会安静检查一次新版本，有新版本时只会提示一次，不会反复打扰。点击提示即可下载对应平台的安装包，覆盖安装就完成升级，你的数据和登录状态都不受影响。'
      },
    ]
  },
  {
