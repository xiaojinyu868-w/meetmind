...
make bench-zhihu-lesson # 收藏夹开课质量门（花模型钱）：同一材料包开 N 节课只听第一轮，量 引材料次数 / 概念命中 / 分歧点名 / 首字延迟；改 teach-live prompt / 节选 / 材料块后跑
...
19. **知乎线（2026-09-12，独立子域名试用）**：用知乎登录 → 选一个收藏夹 → 同学开成一节 live 课 → 讲完就考 → 哪没稳 → 知乎上继续看；最近收藏自动进收集流、知乎画像一条观察进 Hindsight（冷启动）。`src/lib/services/zhihu/`（只读接口客户端 / 页面去杂质 / OAuth 登录绑定 / 收藏夹进收集流 + 最近收藏同步 / 材料包开课（节选按结构）/ 知乎搜索补货 + 模型判断理由 / 画像观察）+ `api/auth/zhihu/*` `api/zhihu/*` + `/apps/zhihu`；teach-live 学会「学生自带材料」（`teach-live/live-materials.ts`，材料段进 system prompt）；知乎搜索作为今日情报并列 provider。**09-13**：知乎退成来源适配器，课的单位 / 分线 / 预算抽成与来源无关的 `services/material-lessons/`；live 课讲完物化成「我的一节课」（`teach-live/lesson-record`，`audioSessions.sourceType='teach-live'`）走录音课同一个复习页（`/app?session=teach:<id>`），复习页挂材料附件层；契约 `docs/plans/2026-09-13-material-lessons.md`。零 schema 改动，`ZHIHU_ENABLED` 灰度；运行在 `/mnt/meetmind-zhihu`（PM2 `meetmind-zhihu` @ 3012，`zhihu.meetmind.online`），合并与否待试用 → `docs/plans/2026-09-12-zhihu-line.md` + `src/lib/services/zhihu/DOMAIN.md` + `src/app/api/zhihu/DOMAIN.md`
...
| **改上课舞台（teach-live：标签流 / 多模态板书 / 节奏）** | `src/lib/services/teach-live/DOMAIN.md`（协议 + 事件）→ `src/components/teach-live/DOMAIN.md`（两条时间轴）→ 协议改 `src/types/teach-live.ts` + `live-markup-parser.ts`；教学大脑改 `src/lib/prompts/teach-live-prompt.ts`（改 `<draw>` API 必须同步 `DRAW_API` 段）；渲染器在 `components/teach-live/blocks/` 与 `draw/`；`<draw>` 运行时 `src/lib/teach-live-draw/DOMAIN.md`（图画不出来先看生产日志 `teach-live-repair` 的 `reported`——是 API 太窄还是模型笔误）；文案 `src/lib/ui/copy-teach-live.ts` |
...
| **改「从材料开一节课」能力层（课的单位 / 分线 / 预算 / 课的物化 / 复习附件层）** | `docs/plans/2026-09-13-material-lessons.md`（三层与边界契约）→ `src/lib/services/material-lessons/DOMAIN.md` → 课的物化 `src/lib/services/teach-live/lesson-record.ts` + `src/lib/db/lesson-records.ts` + `hooks/useLessonRecordSync`；复习页附件层 `src/lib/review/` + `components/review/LessonMaterialsCard`；新来源只写三个薄壳（见 material-lessons DOMAIN「来源怎么接」） |
| **改知乎线（登录 / 收藏夹开课 / 补货）** | `docs/plans/2026-09-12-zhihu-line.md`（北极星与接缝）→ `src/lib/services/zhihu/DOMAIN.md`（知乎接口逐字段事实 + 服务边界）→ `src/app/api/zhihu/DOMAIN.md`（路由 + 页面）→ 材料段在 `src/lib/services/teach-live/live-materials.ts`；文案 `src/lib/ui/copy-zhihu.ts`；验证 `make smoke-zhihu`（需 Secret）/ `make smoke-zhihu-lesson`（不需）/ `make bench-zhihu-lesson`（质量门）；上线 `make deploy-zhihu`（`docs/RELEASE_FLOW.md` 试用实例节） |
