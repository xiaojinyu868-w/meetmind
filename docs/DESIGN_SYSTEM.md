...
## 进入态版式（2026-09-10，应用点开后、成品出来前的那一屏）

用户原话："每一个应用的进入页面都挺难看" / "好的产品应该尽可能少用文字，让用户不看文字就能知道这里是什么意思"。规则与理由：

- **以产物的形状说明自己**（`apps/windows/AppEntrySilhouette`）：一叠牌 = 闪卡，一张有题号的纸 = 测验，几个节点 = 导图，一条波形 = 播客，一块留白的板 = 板书，一张海报比例的框 = 信息图，一张三栏密线的纸 = 速查表。线稿 1.5px 墨色 28% + 纸白 + 极淡投影；等待时内容线轮流呼吸、一处签名色慢慢明灭——这就是"正在做"的状态点，**不用秒数计数器、不用光晕、不用章鱼**（章鱼是同学，产物不是同学）。失败态签名色换朱砂，形状不变。
- **三层，且三种状态（等待 / 空 / 失败）同一套**：形状 → 一句话 ≤14 字（`APPS_COPY.entry`，"在听这节课，出几道题"）→ 一个槽位（等待放原话掠过；空 / 失败放唯一动作）。成品落下来时形状变成真东西，位置不跳。
- **宿主头部一行**：返回 · 应用名 · 动作。第二行的说明句（"检验理解 · 想做题测一测…"）、数据源标签（"示例课"）、分类标签（"记住核心"）全删；状态词只在成品在眼前、正在重做时出现——没有成品时正文自己会说。
- **没有深色房间**：任何单个应用都不开 `--mm-immersive` 之类的沉浸底（闪卡曾有，实测等待态黑底黑字）；白底眩光交给 `data-theme=dark`。
- **需要用户先选的进入态（信息图）同一版式**：框是主角，可选项是框下面两排词（无「尺寸：」标签，框随选择即时变），补一句要求是一行淡字，唯一饱和色给那一个动作。

## 文字密度（2026-09-10）

- 眉题只在确实分隔区块时保留；说明性的句子（"麦克风常开。你讲，他们听…" / "先暴露问题，再记牢…顺序只是建议" / "马上回答，自动接上相关内容"）删；重复的副标题删。
- 状态能用一枚点 / 一个图标表达的就换：做好了不说话；正在做 = 形状里一处签名色明灭。
- 凡是"标签：控件"的排布，改成内容本身可点或两个词的文字开关（问同学第一屏、信息图定制态）。
