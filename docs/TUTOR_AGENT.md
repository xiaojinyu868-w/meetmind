...
goal 模式（M15 起）是**选择题驱动的短仪式**：每轮回复末尾必须带 `---选项---` 快答块（2-4 个 ≤12 字选项，前端渲染果冻按钮），3 轮内必须产出 `---我想要的---` 确认卡，用户保存后一句话收尾。首次会面的第一条用户消息是前端固定开场选择题的答案，模型直接接住，不做自我介绍。
...
- `review`：默认注入 `[MM:SS]` / `[MM:SS-MM:SS]` 合约，前端可以跳回已经保存的原声或转录。注入的转录每行带 `[MM:SS]` / `[HH:MM:SS]` 真实开始时刻（客户端 adapter 按 `formatTimestampLong` 逐段生成，上限 60000 字 ≈ 2.5 小时全量覆盖，超出按播放点行首时间戳定位窗口），模型引用时必须用行首真实时刻、禁止估算或编造。
