...
定义独立成文 `docs/AI_LESSON_DEFINITION.md`：先列 AI 对照人类老师的长板（算得对 / 能动 / 不累 / 记得你 / 知道你听过什么课 / 一整套模型 / 可回看 / 便宜）与短板（看不见你 / 没身体 / 会粗心 / 要等 / 没课堂经验 / 没同学），再给八条"好课"判断与评分卡，最后是系统形态——一位主讲永不阻塞，备课 / 画师 / 审核 / 观察 / 记忆以上下文而非指令进入。§12.4–12.7 的工作属于其中"长板 · 真值可视化"一项；下一步的重心按定义 §6 移到"观察"（把学生信号接进来）与"审核"（复算 + 当众改错）。
