...
## 0.5 进度（2026-09-11 收口时的状态，按线）

| 线 | 状态 | 证据 |
|---|---|---|
| 诚实（无兜底 / 模板产物） | 完成，进 eval-guard | `plugins/DOMAIN.md`、`tests/eval/apps` |
| 问同学 = 同学在场 + 画像栏 + 用户维护画像（curation 事件） | 完成上线 | `components/DOMAIN.md` GlobalAsk / LearnerProfile 行 |
| 应用矩阵交互与表现（八窗口 + 公共壳 + 进入态 + 去衬线斜体） | 完成上线 | `windows/DOMAIN.md` 顶部公共语言 |
| 讲给同桌听 = 面试间（连续讲述 / 评委席 / 隐藏转写 / 停下→出字 1.9s） | 完成上线；出声 2.9s 受 TTS 上游波动 | `windows/DOMAIN.md` 讲给同桌听小节 |
| 课堂线可靠性（录课不丢 / 跨设备可见 / 字幕断连自愈 / 卸载收尾 / 迁移只推变过的） | 完成上线 | `roadmap/v2.1`、`docs/ASR_PIPELINE.md` |
| 内容层第一刀（测验为这个人出 / 闪卡提示式 + 跨会话到期 / 评委一句话一个问题） | 完成上线 | `tests/eval/apps/DOMAIN.md` |
| 内容层第二刀（板书脚本讲法 / 讲给同桌听评估标准 / 播客与信息图密度） | **未开始** | — |
| 数据目录迁出开发目录（`/mnt/meetmind-data`） | **待办**（维护窗口） | `docs/RELEASE_FLOW.md` |
