...
| **基建** | check 绿、CI 刚修通、eval 三套 | lint 178 红；72 个超预算文件；双 lockfile；埋点 EventTrack 5 月后停更（`tutor_chat_start` 之外无事件） | **先把埋点接回来**——没有埋点，本文以后写不出来 |
...
（由浏览器走查产出，见后续追加；覆盖首页 → 试听课 → 结课 → 复习三栏 → 应用矩阵逐个 → 移动视口 → 设置 / 我的上下文。）
