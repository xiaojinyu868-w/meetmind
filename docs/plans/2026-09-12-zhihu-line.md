...
- **09-13 课的单位改成一篇 / 一条线（产品负责人指出："一次讲四篇或 98 篇显然不对，选部分内容来讲的机制是开玩笑"）**：真实收藏夹是杂的（93 条里有暑期工的想法和 dLLM 的回答），"一个收藏夹 = 一节课"不成立，"按赞同前 8 篇各 1800 字"是替用户做了一个他没被告知的决定。改为：收藏夹页 → 同学读出几条线（模型一次调用、缓存）→ 每篇「讲这篇」（全文）/ 每条线「按这条线开一节」（2–4 篇）/ 顶部「随手开一节」（同学挑一篇并说为什么，零选择的默认保留但说清它选了什么）。整夹开课退成兜底 / smoke。
