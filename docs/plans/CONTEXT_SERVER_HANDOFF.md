...
## 服务器接收记录（2026-09-09，服务器 AI）

- **接收方式**：`git worktree` 隔离目录从 `feat/settings-redesign`（1ee8770）合并 `origin/feat/context-m1-handoff`（9aa9133，基线 5326eea 之后
  我方 37 个提交）。7 处冲突：Makefile（保留 Node 24 自动探测 + 加 context/hindsight target）、`.env.example`、ledger、AppRenderSurface、
  QuizWindow（保留服务器版底部自评条，接 `recordAttempt` 观察）、useAppLearningActivity（保留 `recordAssessment`，接 M1 的 observation /
  登录排队）、types DOMAIN。pnpm 是唯一锁：新增 `@modelcontextprotocol/sdk`、显式 `@next/env`（pnpm 不提升，worker / smoke 直接 import）。
- **有意偏离**：① 写侧由"CONTEXT_ENABLED 时只写新存储"改为**双写**——服务器版在 9-8 已把 `LearningEvent` 变成掌握轨迹 / LearnerContext 事实半的
  原料，二选一会让开启 Context 反而断掉这些读侧；② 教育适配器补 assessment 载荷（原 strict schema 会把它 400 掉）；③ Tutor 消费改走
  `LearnerContext.understanding`（同一 helper `learner-understanding`），global Tutor 不再双注入，`tutorContextSuffix` 保留供回归测试；
  ④ smoke 的浏览器 channel 默认 msedge，服务器用 `SMOKE_BROWSER=chromium`。
- **验证**：`prisma db push` 副本库只增 ContextEvent / ContextGrant；`tsc` 绿；vitest 204 文件 1471 用例全过（context 26）；`make build` 通过；
  隔离服务 3107 + `make context-worker` + 真 Hindsight：`smoke-context-live` 10/10 通过（报告 `out/context-live/<fixture>/result.json`）；
  读写归一探针（课堂观察 + assessment → worker completed → 事实 2 + 理解 2 来源 → execute trace `server:2+memory`）通过，合成账号清理完成。
- **部署**：生产 `.env` 加 `CONTEXT_ENABLED=true` / `CONTEXT_HINDSIGHT_URL=http://127.0.0.1:18888` / `CONTEXT_HINDSIGHT_API_KEY`（取自独立服务配置）；
  `make db-push`（只增表）；PM2 新进程 `meetmind-context-worker`（`npx tsx src/lib/services/context/worker.ts`，与 Web 同 cwd / 同库）；
  `make deploy`。回滚：`CONTEXT_ENABLED=false` 关内部应用读写，worker 继续完成已排队清理，表不删。
- **未做**（按原文顺序仍开着）：旧「我的上下文」迁到 `/context` 画像页；离线 outbox；SDK / MCP 发布与 OAuth；生产 Hindsight 外置 PostgreSQL 与备份。
