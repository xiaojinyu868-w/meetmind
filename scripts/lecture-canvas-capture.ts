/**
 * lecture-canvas-capture.ts —— v31 白纸讲义画布验收：加速播放全程连拍
 * 每 2s 截一张；遇到 checkpoint 自动点「看解析」推进；
 * 翻到最后一页且无 checkpoint 待答后收尾。取每页翻页前最后一帧 = 成品态。
 * 用法：npx tsx scripts/lecture-canvas-capture.ts
 */
import { chromium } from 'playwright';
import { mkdirSync } from 'node:fs';

const BASE = 'http://localhost:3101/demo-board?script=board-script-agent.json&pace=40';
const OUT = 'out/lecture-canvas-v31';
...
async function main() {
  ...
  // headless 通常自动开播；没有就点播放（精确匹配按钮文案，防误触）
  const playBtn = page.getByRole('button', { name: '播放', exact: true });
  ...
  while (Date.now() - t0 < 480000) {
    ...
    if (!checkpointClicked) {
      const reveal = page.getByRole('button', { name: '看解析', exact: true });
    ...
    if (match && match[1] === match[2]) {
      const pauseCount = await page.getByRole('button', { name: '暂停', exact: true }).count();
      const resumeCount = await page.getByRole('button', { name: '播放', exact: true }).count();
      if (pauseCount === 0 && resumeCount === 0 && Date.now() - lastPageSeenAt > 8000) {
        // 收尾段全部动作锚在讲稿末尾：status 到 finished 时末页书写还在排空，
        // 多等 25s 让串行链写完 + 圈点勾画落笔，再拍最终成品帧
        console.log('final page finished, waiting 25s for ink drain');
        await page.waitForTimeout(25000);
        idx += 1;
        await page.screenshot({ path: `${OUT}/seq-${String(idx).padStart(3, '0')}.png` });
        console.log('final settled shot taken');
        break;
      }
