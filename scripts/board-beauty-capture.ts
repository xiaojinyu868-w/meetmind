/**
 * board-beauty-capture.ts —— 板书美观性审查：加速播放全程连拍
 * 每 2s 截一张；遇到 checkpoint 自动点「看解析」推进；
 * 翻到最后一页且无 checkpoint 待答后收尾。取每页翻页前最后一帧 = 成品态。
 * 用法：npx tsx scripts/board-beauty-capture.ts
 */
import { chromium } from 'playwright';
import { mkdirSync } from 'node:fs';

const BASE = 'http://localhost:3101/demo-board?pace=40';
const OUT = 'out/beauty';
