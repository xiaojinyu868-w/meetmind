...
## 2026-09-11 — 讲给同桌听像通话：停下 → 出字 1.9s、出声 2.9s（原 2.8s / 5.2s），不把换气当讲完

上一轮把评委一次开口压到五六秒，但"你停下 → 听见评委开口"仍要 5~8s。这一轮先搭尺子再改：`teach-back:timing` 打点通道（`teach-back-timing.ts`，对齐 `board:timing`，生产零开销）+ Playwright 假麦克风灌 5 段学生讲述、逐段量 最后一帧有声 → 提交 → SSE 首字 → 出字 → 首句 → TTS 首片 → 出声。基线（5 回合均值 / 最差）：停下 → 出字 2798 / 3840ms，→ 出声 5204 / 7084ms；改后 1881 / 2167ms，2908 / 3684ms。三段各自的改法与数据：

- **回合判定**（`teach-back-turn-machine` v2）：不再等 ASR 定稿——实测 qwen-audio-3.0 流式 ASR 在你停下那一刻 interim 已是整句，定稿却要 1.2~1.4s 才到，且 `max_sentence_silence` 500 / 300 / 1000 无差别；静音到点就拿 interim 提交，迟到的定稿 `turn-upgrade` 只替换尾巴（多句回合前面的定稿句保留）。门槛没有压到 0.8s：原始音频里句内停顿最长 660ms，但经浏览器采集链后客户端 VAD 看到 670~1130ms，900ms 实测 15 段 2 次假回合（评委开口又被打断、记录里多一行半截话）——**误判换气比慢更糟**，所以留在 1.2s，尾巴像没说完（逗号 / 连词 / 语气词 / 介词收尾，`looksUnfinished`）再多等半秒到 1.7s；延迟靠 **停下 650ms 就预热评委请求**（`turn-prefetch`，事件攒着不上屏，提交时文字对得上 `prefetchCovers` 就接上、接着讲就 abort）：提交 → 首字从 1683 / 2543ms 到 779 / 968ms。改后 4 轮 20 段 0 假回合。ASR 代理支持按连接申明 `vadSilenceMs` / `draftFlushMs`（`resolveTurnTuning`，课堂录音默认不变）。
- **模型首字**：flash 档实测不换——qwen3.6-flash 首字 0.58s 但 5 回合 3 次沉默、回应对不上刚讲的段；DeepSeek-V4-Flash 首字 ~1.0s 不比 qwen3.7-plus 快、10 回合 4 次沉默；step-3.7-flash 额度 402。评委上下文 9k → 5k：原文改为与刚讲这段相关的窗口（`selectRelevantTranscript`：命中段 + 邻居 + 目标点证据段，整段原话不截，此前超预算是逐段压成半截），历史只留最近两回合；首字对原文 8k / 5k / 2.5k 不敏感（1.09 / 1.12 / 0.96s，噪声内），收预算是为原话完整可核对与成本；prompt 人设不动，只在节选时注明"节选之外不要断言老师没讲过"（可见代价：不再有"原文没提双射这个词"这类判断）。
- **TTS 首句**：`/api/teach/tts` 新增 `stream:true`——百炼 qwen3-tts 开 SSE 边合成边发 base64 PCM（首片 0.4s，原来等整句合成完再回源 2.2s+），服务端转成 audio/pcm 分片、整段收齐后仍进 wav 缓存（缓存命中也流式发）、上游流式不可用静默退回 wav；前端 `pcm-stream-audio.ts` Web Audio 句柄首片到就调度、首尾相接，`TeachSpeechPlayer` 两种源不区分。首句文本齐 → 首片从 2115 / 2920ms 到 637 / 1410ms。
- 文字先到先出字（首字 → 出字 3ms）、声音后到再出声，本来就是并行的；提交后即发请求也已是；这次把"发请求"再提前到停顿那一刻。
- 单测：状态机 34 例（预热 / 取消 / 尾巴语义 / 多句升级）、`pcm-stream-audio` 5 例、`teach-tts-stream` 5 例、`selectRelevantTranscript` 5 例、`resolveTurnTuning` 3 例、`appendWsQuery`。评委回答质量对照（改前后各 5 回合）与分段表在 `src/components/apps/windows/DOMAIN.md`「延迟尺子」。
...
## 2026-09-13 — 课的单位、课的物化、能力层：知乎退成来源，「从材料开一节课」成为产品能力

- **课的单位改成一篇 / 一条线**（产品负责人：一次讲 4 篇或 98 篇不对）：收藏夹页 → 同学把收藏读成几条线（一次模型调用，按条目集合缓存）→
  每篇「讲这篇」（全文 ≤1.2 万字整篇给老师，再长按结构取并附目录）/ 每条线「按这条线开一节」（2–4 篇）/ 「随手开一节」（同学挑一篇并说为什么）；
  整夹开课退成兜底。线上 deeplearning 实听：一条线课「你收藏的两篇文章正好是这条线……」并记得上节课；单篇课「把这篇讲透，从第一个环节开始」。（54044ac）
- **[core] 课的物化**：live 课讲完 → `LessonRecord`（老师 / 学生转录段、页、材料、之前的课一行、digest）→ IndexedDB 一节课（`sourceType='teach-live'`）→
  `/app?session=teach:<id>` 直达 MeetMind 原来的复习页；讲完一轮右下「去复习」。迷你课后页退役。（0653f6e / 5c39323）
- **[core] 复习附件层**：复习页时间轴顶部材料卡（讲的是哪几篇 / 老师读了全文还是骨架 / 讲到没讲到 / 看原文 / 接着讲）；复习同桌拿到这节课点名的几篇（≤9000 字，
  走既有 supportMaterials 接缝）；知乎「继续看」搬进复习页，输入是这节课复习的交卷记录。（1e4fd87）
- **[core] 能力层 `services/material-lessons/`**：候选材料形状、课的单位与预算、分线、接下来看哪篇的判断——与来源无关；知乎只剩 capture → candidate 的薄壳。
  契约 `docs/plans/2026-09-13-material-lessons.md`：来源 / 能力 / 体验三层、来源五接口、模型看什么的边界表（一次调用 = 一个场景 + 点名的依据 + 这个人 + 邻居一行）、赛后要按此改造的四处。（a3c32e6）
- **OAuth 按黑客松版定稿**：凭证来自活动页创建项目（App ID 424 已配，`oauthReady=true`）；`/user` 两种鉴权形态都认、uid 无损、hash_id 作身份、state 回传才比对。
  额度实测低额度档（搜索 10+10 / 天）：缓存 + 预算门 + 情报补货默认关 + 并发串行 + 收藏夹清单缓存。
- 全量 1893 例单测通过；共享文件（God File / ReviewWorkspacePanel / DesktopVideoReviewLayout / useReviewSession）eslint warning 零新增。
- 已知未做：手机端复习页（MobileAppShell）暂无材料卡；跨设备的服务端 TranscriptSegment；unit pack / 问同学 / teach-live 对话史按边界表改造（赛后）。

## 2026-09-12（晚） — 知乎线打磨：从 Demo 到产品——前端先立起来，再进内容层，再补两条原意

- **哪里还是 Demo（自审）→ 全部做掉**：第一屏三种状态各一句人话（桌宠 + 三步预期；未登录只给能用的按钮——OAuth 凭证没到就不出现「用知乎登录」，
  公开接口 `GET /api/auth/zhihu/status` 决定；收藏夹带状态"已收下 N 条 / 读了全文 / 开过课 / 回到上次那节"；「开课」三步进度真实可见；「你开过的课」）；
  课堂页老师**讲完一轮那一刻**右下长出「讲完这一段了。考一考？／继续听」（机器 act）；新的**课后三栏页** `/apps/zhihu/lesson/[id]/review`（左材料有根、
  中测验闪卡、右继续看；进来就自动出题）；「回到课堂 0:07」→「看这篇材料：A1《…》」（`QuizWindow` / `FlashcardsWindow` 加可选 `evidenceLabel`，默认不变）。（0f8078a）
- **内容层三刀**：节选按结构取（开头 + 每个小标题及首段 + 结尾，真实 19K 字回答 6 小节全带上）；题目解析里的假时间「0:07-0:21 明确指出」渲染前换成
  真材料「材料 1《过拟合到底是什么？》明确指出」；「继续看」每组先留 6 条候选，快模型针对**这个学生刚没稳的那个概念**挑 1–2 条并写一句只落在摘要里的理由，
  失败退回元数据理由。（1db130d）
- **原意补齐**：`POST /api/zhihu/sync` 最近收藏自动进收集流（零动作入口，15 分钟节流，线上 50 条 2.4 s）；知乎画像（收藏夹 / 最近收藏 / 关注 / 创作）
  写成**一条**有来源无判断的学习观察，双写 activity 事件 + `zhihu.profile` ContextEvent 进 Hindsight——新用户第一天同学就知道他在学什么（542cfd0）；
  同一收藏夹再开一节时材料包记 `priorLessons`，老师不从头讲同一段（5e5f785）。
- **质量门**：`make bench-zhihu-lesson`（同一材料包开 N 节只听第一轮：引材料次数 / 概念命中 / 分歧点名 / 首字延迟）；线上基线 首字 ≈2 s、
  平均 5.3 次「材料 N」点名、3/3 讲完。真实收藏夹「deeplearning」线上全程：4 篇全文（最长 28K 字）→ 34 s 开课 → 老师第一轮 636 字从 Word2Vec 材料讲起。
- **顺手**：服务器磁盘满到 100%（一次构建因此失败，线上没受影响）——清了 pip / npm 缓存、pnpm 未引用包、旧 chromium、/tmp 里一个月前的 venv 与 pytest 产物，
  回到 95%；大头（docker 13G、`.vscode-server` 5.3G、`personal-site` 5.7G、`meetmind_go` 4.1G、主目录 `.next-attr` / `.next-preview` 2.5G、`out/` 1.9G）留给负责人决定。
- 未做 / 留给主干：ai-native prompt 层的来源无关化（现在靠渲染层重标注）；SharedAgent 分享一节收藏夹课；OAuth 真实授权仍等 `app_id / app_key`。
