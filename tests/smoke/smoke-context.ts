...
async function main(): Promise<void> {
  ...
  try {
    ...
    await page.getByText(COPY.sharedContext.settings, { exact: true }).click();
    const appInput = page.getByLabel(COPY.sharedContext.appId, { exact: true });
    ...
    await page.getByLabel(COPY.sharedContext.task, { exact: true }).fill('帮我安排一次条件概率练习');
    await page.getByRole('button', { name: COPY.sharedContext.prepare, exact: true }).click();
    await page.getByTestId('context-task-result').getByText(COPY.sharedContext.degraded, { exact: true }).waitFor();
    ...
    await page.getByRole('button', { name: COPY.sharedContext.pause, exact: true }).click();
    await page.getByRole('button', { name: COPY.sharedContext.resume, exact: true }).waitFor();
